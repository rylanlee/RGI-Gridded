...
def grid_glacier_area(gla, grid):
    ...
    error = np.sum(3*0.034*((grid_gla_proj['geometry'].area/1e6)**0.7))
    grid_['area'] = area
    grid_['error'] = error
    print(t)
    return grid_
...
if __name__=='__main__':
    ...
    path = '/media/iceman/Data/00_rgi60'
    rgi_folder = os.listdir(path)
    gla_path = [os.path.join(path, i) for i in rgi_folder if '.shp' in i][10:]
    for i in gla_path:
        ...
        outpath = '/media/iceman/Data/rgi_grid_error/'+i[28:]
